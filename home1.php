...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="home1.php"><i class="fa fa-list"></i> View All Universities</a></li>
      </ol>
    </section>
    ...
    <section class="content">
		...
		$query = "SELECT COUNT(surname) as 'total' from data where university = 'Ateneo De Manila University'";
		$result=mysqli_query($dbc,$query);
		$row=mysqli_fetch_array($result,MYSQL_ASSOC);
											
		echo "<font size = '5'><b>ADMU: </b>". $row['total']. "</font> &emsp;";
		
		$query = "SELECT COUNT(surname) as 'total' from data where university = 'De La Salle University'";
		$result=mysqli_query($dbc,$query);
		$row=mysqli_fetch_array($result,MYSQL_ASSOC);
											
		echo "<font size = '5'><b>DLSU: </b>". $row['total']. "</font> &emsp;";
		
		$query = "SELECT COUNT(surname) as 'total' from data where university = 'University of the Philippines'";
		$result=mysqli_query($dbc,$query);
		$row=mysqli_fetch_array($result,MYSQL_ASSOC);
											
		echo "<font size = '5'><b>UP: </b>". $row['total']. "</font> &emsp;";
		
		$query = "SELECT COUNT(surname) as 'total' from data where university = 'University of Santo Tomas'";
		$result=mysqli_query($dbc,$query);
		$row=mysqli_fetch_array($result,MYSQL_ASSOC);
											
		echo "<font size = '5'><b>UST: </b>". $row['total']. "</font> &emsp;";
		
		$query = "SELECT COUNT(surname) as 'total' from data where university = 'Lyceum of the Philippines University'";
		$result=mysqli_query($dbc,$query);
		$row=mysqli_fetch_array($result,MYSQL_ASSOC);
											
		echo "<font size = '5'><b>LPU: </b>". $row['total']. "</font> &emsp;";
	
		$query = "SELECT COUNT(surname) as 'total' from data where university = 'Systems Technology Institute'";
		$result=mysqli_query($dbc,$query);
		$row=mysqli_fetch_array($result,MYSQL_ASSOC);
											
		echo "<font size = '5'><b>STI: </b>". $row['total']. "</font> &emsp;";
						...
						<tbody>
							...
							if (isset($_POST['submit'])){
								...
								if(empty($_POST['startAge']) || empty($_POST['endAge'])){
									echo "<font color=\"red\">incomplete age range!!!</font>";
								} else{
									for($i=0; $i < $count; $i++){
									require_once('../mysql_connect.php');
									$age = "SELECT (YEAR(curdate()) - YEAR(birthday)) as 'age' from data";
									$query = "SELECT surname, name, birthday, university, (YEAR(curdate()) - YEAR(birthday)) as 'age' from data where university = \"".$aUniv[$i]."\" and (YEAR(curdate()) - YEAR(birthday)) >= \"".$startAge."\" && (YEAR(curdate()) - YEAR(birthday)) <= \"".$endAge."\"";
									$result=mysqli_query($dbc,$query);
